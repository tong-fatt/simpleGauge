var needle = [];
var barWidth, chart, chartInset, degToRad, repaintGauge, el,
...
function createGauges(worksheetData)
	{   for (i = 0 ; i< 3; i++){
        ...
        needle[worksheetData._data[i][0]._value] = new Needle(chart, width, worksheetData._data[i][1]._value);
		needle[worksheetData._data[i][0]._value].moveTo(el, worksheetData._data[i][1]._value, 0.3, 1,
			worksheetData._data[i][0]._value);    
			console.log('worksheetData:' + JSON.stringify(worksheetData._data[i][0]._value));              
		}
    ...
    */
    ...
    var Needle = (function () {
        ...
        **/
        ...
        Needle.prototype.moveTo = function (el, act, tgt, max, lbl) {
            var perc = act / max;
            var perc2 = tgt / max; 
                self,
            ...
            this.el = el;
            self = this;
            console.log('e1 before tween:: '+ JSON.stringify(this.el))
            ...
            this.el.transition().delay(100).ease('quad').duration(200).select('.needle').tween('reset-progress', function () {
                return function (percentOfPercent) {
                    ...
                    return 
                    d3.select(this).attr('d', recalcPointerPos.call(self, progress));
                   
                };
            });
              
            console.log("select needle");
            console.log('e1 before tween::: '+ JSON.stringify(el))
            el.transition().delay(300).ease('bounce').duration(1500).select('.needle').tween('progress', function () {
                return function (percentOfPercent) {
